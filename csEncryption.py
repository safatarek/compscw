...
def sBoxSub(bits48):
    ...
    for SboxIndex in range(8):
        block6=bits48[SboxIndex*6:SboxIndex*6+6] #divides into 8s

        RowBits=block6[0]+block6[5]
        row=int(RowBits, 2) #tels python that this is binary then converts into deicmal
...
def EncryptBlock(block64b, roundkeys):
    ...
    R=IPresult[32:]
    for i in range(16):
        NewL=R #left part for the next round is the current right half
        fFoutput=fFunction(R, roundkeys[i]) #index rounding
        NewR=XOR(L, fFoutput)
        L=NewL #old right
        R=NewR #loop=data is mixed and ready for the next part
    swap=R+L
...
def TextToBits(text8):
    ...
    bits=''
    for chrctr in text8:
        asciiValue=ord(chrctr) #converts from characters to decimal number
        binary8b=format(asciiValue, '08b') #8 bits and padding with zeros
        bits+=binary8b
    return bits
def BitsToText(bits):
    text=''
